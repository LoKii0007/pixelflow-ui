...
const MobileMenuItem = ({ label, href, activeView }) => {
  return (
    <Link className={`relative group overflow-hidden`} href={href}>
      <div
        className={`group-hover:text-black transition-all duration-300 z-20 relative pe-5 ${
          activeView === href
      ...
      <div
        className={`w-0 h-full absolute top-0 right-0 bg-white transition-all duration-300 ${
          activeView !== href ? "group-hover:w-full" : ""
    ...
    <div className="flex justify-center items-center absolute top-0 left-0 w-full z-10">
      ...
      <div
        className={`nav-mobile ${
          isSidebarOpen ? "md:w-2/3 xl:w-1/2 w-full opacity-100" : "w-0 opacity-0"
