...
export function Accordion00LyraDemo() {
    const [openIndex, setOpenIndex] = React.useState(0); // Default open 0 to match Nova behavior
    ...
    return (
        ...
        >
            {demoOptions.map((item, i) => {
                ...
                // When no item is open (`openIndex === null`), disable the adjacency borders
                // so we just rely on standard borders.
                ...
                return (
                    <AccordionItem
                        ...
                        value={`item-${i}`}
                        className={cn(
                            // Only apply adjacency borders if something is actually open to push items around.
                            // If NO item is open, base styling handles it.
...
const AccordionItem = React.forwardRef(({ className, ...props }, ref) => (
    <AccordionPrimitive.Item
        ...
        className={cn(
            ...
            // BASE STATE:
            // - Left/Right borders always present
            // - Top/Bottom borders: "first:border-t" and "border-b-0" logic works for adjacencies,
            //   BUT "last:border-b" ensures the list closes.
            "border-b-0 border-l border-r border-white/20 first:border-t last:border-b",

            // OPEN STATE:
            // - Pop out with margin (my-4)
            // - Full 4-side border (border)
            // - Highlighted border color (border-white/40)
